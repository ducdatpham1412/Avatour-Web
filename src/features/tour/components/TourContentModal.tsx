...
const TourContentModal = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={toggleDialog}>
      ...
      <DialogContent
        closeButton={<></>}
        className="!rounded-[16px] py-11 px-10 !max-w-[calc(100vw_-_40px)] md:!max-w-[40vw] !max-h-[80vh]"
      >
        ...
        <div className="flex flex-col gap-y-6">
          <h1 className="text-[32px] leading-[44px] font-normal">{title}</h1>
          <p className="text-[16px] leading-[28px] font-light">{content}</p>
        </div>
      </DialogContent>
    </Dialog>
  );
