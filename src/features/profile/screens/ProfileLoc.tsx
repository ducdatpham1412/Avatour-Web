...
const InfoBlock = ({
  ...
  return (
    <div
      className={cn(
        'w-full p-[16px] border-[1px] border-gray_300 rounded-[12px] inline-flex gap-[20px]',
        className,
      )}
      style={{ flexDirection: flex, alignItems: flex === 'row' ? 'center' : 'flex-start' }}
    >
      <div
      ...
      <p>{desc}</p>
      {!!moreDesc && <p>{moreDesc}</p>}
    </div>
  );
};

const Card = ({ children }: PropsWithChildren) => {
  return <div className="inline-flex items-center w-[30%] mr-2">{children}</div>;
};
...
const BlockCard = ({ title, data, renderItem, BottomElement }: BlockCardProps) => {
  return (
    <Block>
      <Title title={title} />
      <div className="w-full inline-flex flex-wrap">{data.map(renderItem)}</div>
      {BottomElement}
    </Block>
  );
};
  ...
  return (
    <main className="w-full pb-[100px]">
      <NameStars
        ...
        ratings={0}
        className="mt-[24px]"
        nameClassName="md:text-[30px]"
      />

      <div className="w-full inline-flex gap-x-[16px] px-[2px] mt-[24px] h-[40vw]">
        <Image
      ...
      <div className="w-full inline-flex flex:col lg:flex-row gap-x-[100px] mt-[48px]">
        <div className="w-[62%] inline-flex flex-col gap-[24px]">
          ...
          <Block>
            ...
            <div className="inline-flex gap-[10px]">
              ...
              <div>
                <p>{data.location}</p>
                {!!data.info.gg_map && (
                  <Link className="underline font-light" href={data.info.gg_map} target="_blank">
                    Xem bản đồ
                  </Link>
                )}
              </div>
            ...
            <div className="relative inline-flex gap-[16px]">
              <InfoBlock
              ...
              />
              {!!data.info.best_time && (
                <InfoBlock desc={data.info.best_time} className="w-auto" flex="column" />
              )}
            </div>
          </Block>
