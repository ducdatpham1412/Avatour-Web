...
const ItemVoucher = ({ item, onClick, status, hasChosen = false }: Props) => {
  ...
  return (
    <button
      className={cn(
        'bg-white py-[12px] px-[16px] rounded-[14px] border-[1px] border-gray_200 shrink-0',
    ...
    >
      ...
      <div className="border-t-[1px] border-t-gray_400 border-dashed my-[12px]" />
      ...
      {item.status === 'used' && <p className="text-[12px] text-start mt-[12px]">(Đã sử dụng)</p>}
    </button>
  );
};
