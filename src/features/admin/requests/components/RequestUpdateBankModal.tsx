...
const RequestUpdateBankModal = withRequest<'update_bank'>(
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={e => openOpenChange(e)}>
        <DialogContent
          ...
          className="!w-min min-w-[450px] h-min xl:max-h-[calc(100vh_-_40px)] xl max-w-full max-h-full bg-white gap-3 overflow-hidden !rounded-2xl"
        >
...
const UpdatePriceItem = ({ data, type }: { data: UpdateBank; type: 'old' | 'new' }) => (
  <div className="flex flex-col p-4 bg-gray-50 rounded-[16px] w-full gap-2">
    <span className="font-bold">{type === 'new' ? 'Cập nhật' : 'Hiện tại'}</span>
    <div className="flex items-start gap-2">
      <ul className="list-[disc] pl-[30px] flex flex-col gap-1">
        <li>Ngân hàng: {data.bank_code}</li>
        <li>Số tài khoản: {data.bank_account}</li>
      </ul>
    </div>
  </div>
);

export default RequestUpdateBankModal;
